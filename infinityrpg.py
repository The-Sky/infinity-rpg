###################################################################################
#                                                                                 #
# InfinityRPG by Caffeine                                                              #
#                                                                                 #
# Version 1.0.0                                                                   #
# Edited/Fixed version of Source/Meow RPG                                                                                #
###################################################################################
...
# Main paths
pluginPath = os.path.join(os.getcwd(), 'cstrike/addons/eventscripts/infinityrpg')
configPath = os.path.join(os.getcwd(), 'cstrike/cfg/infinityrpg')
...
class Config(object):
    '''This class provides access to the configuration of InfinityRPG.
    
    You should not create any instance of this class. A instance called "config" is available in the InfinityRPG core script.

    Example:

    >>> from infinityrpg import config
    ...
    def GetString(self, key):
        ...
        You can find valid keys in the configuration files, which can be found in the folder "cstrike/cfg/infinityrpg".'''
...
class Internet(object):
    ...
    def Connect(self):        
        try:
            ...
            self.connected = True
            es.server.queuecmd('echo "[InfinityRPG] Connected to Master Server"')
        except:
            self.connected = False
...
class Skillhandler(object):
    '''This class provides access to all Skills of InfinityRPG.
    
    You should not create any instance of this class. A instance called "skillhandler" is available in the InfinityRPG core script.

    Example:

    >>> from infinityrpg import skillhandler
    ...
    def LoadAll(self):
        ...
        for i in [i for i in self.skills.keys() if not self.skills[i]]:
            if config.GetBool('rpg%sLoad' %(i)):
                es.server.queuecmd('es_xload infinityrpg/Skills/%s' %(i))
                self.skills[i] = True   
    ...
    def Load(self, skill):
        ...
        if skill in self.skills.keys():
            if not self.skills[skill]:
                es.server.queuecmd('es_xload infinityrpg/Skills/%s' %(skill))
    ...
    def UnloadAll(self):
        ...
        for i in [i for i in self.skills.keys() if self.skills[i]]:
            es.server.queuecmd('es_xunload infinityrpg/Skills/%s' %(i))
            self.skills[i] = False  
    ...
    def Unload(self, skill):
        ...
        if skill in self.skills.keys():
            if self.skills[skill]:
                es.server.queuecmd('es_xunload infinityrpg/Skills/%s' %(skill))
...
class Ranking(object):               
    ...
    def SendPopup(self, userid):
        ...
        popup.addline('[InfinityRPG] Top 10')
        for i in xrange(1,11):
            data = saving.GetRank(i)
            if data != None:
                popup.addline('%s. %s - Level %s' %(i, data[0], data[2]))
...
class MainPopup(object):
    ...
    def CreatePopup(self, first=False):  
        ...
        self.popup.addline('[InfinityRPG] Main')
...
class UpgradePopup(object):
    ...
    def CreatePopup(self, first=False):  
        ...
        # Create new popup     
        self.popup = MultiPopup(self.language, self.popupname, ('[InfinityRPG] %s' %(language.GetLanguage('upgrade', self.language)), language.GetLanguage('current_credits', self.language, {'credits' : self.player.GetCredits()})), self.Menuselect) 
  
        for i in skillhandler.GetLoadedSkills():
            if not self.player.IsSkillMax(i):
                self.popup.AddOption(language.GetLanguage('upgrade_level', self.language, {'skill' : i, 'level' : self.player.GetSkillLevel(i), 'cost' : self.player.GetSkillCredits(i), 'max' : self.player.GetMaxLevel(i)}), i, self.player.HasSkillCredits(i))
...
class SellPopup(object):
    ...
    def CreatePopup(self, first=False):  
        ...
        # Create new popup     
        self.popup = MultiPopup(self.language, self.popupname, ('[InfinityRPG] %s' %(language.GetLanguage('sell', self.language)),  language.GetLanguage('current_credits', self.language, {'credits' : self.player.GetCredits()})), self.Menuselect) 
        
        for i in self.player.GetSkills():
            self.popup.AddOption(language.GetLanguage('sell_level', self.language, {'skill' : i, 'level' : self.player.GetSkillLevel(i, False), 'cost' : self.player.GetSkillSellCredits(i), 'max' : self.player.GetMaxLevel(i)}), i)    
...
class StatisticPopup(object):
    ...
    def CreatePopup(self, first=False):  
        ...
        # Create new popup
        self.popup = popuplib.create(self.popupname)
        self.popup.addline('[InfinityRPG] %s' %(language.GetLanguage('statistic', self.language)))
...
class SettingsPopup(object):
    ...
    def CreatePopup(self, first=False):  
        ...
        self.popup = popuplib.create(self.popupname) 
        self.popup.addline('[InfinityRPG] %s' %(language.GetLanguage('settings', self.language)))
...
class LanguagePopup(object):
    ...
    def CreatePopup(self, first=False):  
        ...
        self.popup = MultiPopup(self.language, self.popupname, ('[InfinityRPG] %s' %(language.GetLanguage('language_headline', self.language)),  language.GetLanguage('language_current', self.language, {'language' : language.GetLanguageByShortcut(self.language)})), self.Menuselect) 
...
class InfoPopup(object):
    ...
    def CreatePopup(self, first=False):  
        ...
        self.popup = popuplib.create(self.popupname)
        self.popup.addline(' [InfinityRPG] %s' %(language.GetLanguage('information', self.language)))
        self.popup.addline("->-----------------------------------")
        self.popup.addline('->           Version: %s' %(version)) 
        self.popup.addline("->  Scripter: Caffeine's sexy ass.")
        self.popup.addline("->-----------------------------------")
        self.popup.addline("-> This was a shitty RPG  that was")
        self.popup.addline("-> edited to make it actually work.")
        self.popup.addline("->-----------------------------------")
        self.popup.addline("->   Don't try this at home kids.")
        self.popup.addline("->-----------------------------------")
        self.popup.addline('->8. %s' %(language.GetLanguage('back', self.language)))
        self.popup.addline('->0. %s' %(language.GetLanguage('exit', self.language)))
        self.popup.menuselect = self.Menuselect
    ...
    def Menuselect(self, userid, choice, id):
        if choice == 1:
            usermsg.motd(userid, 2, 'Meow-RPG Skills', 'http://meow-rpg.com/skills.html')
        elif choice == 2 and config.GetBool('rpgPremium'):
            usermsg.motd(userid, 2, 'Meow-RPG Premium', 'http://meow-rpg.com/premium.php')  
...
class AdminPopup(object):
    def __init__(self): 
        ...
        self.basicPopup.addline('[InfinityRPG] Admin') 
        ...
        self.xpAmountPopup = MultiPopup('en', 'rpg_admin_amount', ('[InfinityRPG] Admin', 'Select an amount of XP'), self.MenuselectAmount) 
        for i in (100,500,1000,2000,5000,10000,50000,100000,500000):
            self.xpAmountPopup.AddOption(str(i), i)
            
        self.levelAmountPopup = MultiPopup('en', 'rpg_admin_amount', ('[InfinityRPG] Admin', 'Select an amount of Level'), self.MenuselectAmount) 
        for i in (1,5,10,15,20,25,50,100,500,1000,5000,10000):
            self.levelAmountPopup.AddOption(str(i), i)
            
        self.creditsAmountPopup = MultiPopup('en', 'rpg_admin_amount', ('[InfinityRPG] Admin', 'Select an amount of Credits'), self.MenuselectAmount) 
        for i in (1,5,10,15,20,25,50,100,500,1000,5000,10000):
    ...
    def CreatePlayerPopup(self, userid):  
        ...
        popup = MultiPopup('en', 'rpg_admin_player', ('[InfinityRPG] Admin', 'Select a Player'), self.MenuselectPlayer)
        for i in playerlist.GetPlayerlist():
...
class Saving(object):
    def __init__(self, path): 
        ...
        parser.read('cstrike/addons/eventscripts/infinityrpg/database.ini')

        # Set our databse information from the database.ini
    ...
    def Debug(self):
        ...
        for i in data:
            tmp.append(str(i[0]))          
        for i in tmp:  
            self.Execute('''UPDATE rpg_version_1_0_1 SET rank = %i WHERE steamid = "%s"''' % (cnt,i))
            cnt += 1   
    ...
    def Load(self, player):
        ...
        self.Execute('''SELECT * FROM rpg_version_1_0_1 WHERE steamid = "%s"''' % (playersteamid))
        data = self.Fetchall()
        ...
        if not found:
            ...
            self.Execute('''SELECT COUNT(*) FROM rpg_version_1_0_1 WHERE totalxp >= %i''' % (player.totalxp))
            data = self.Fetchall()
            rank = int(data[0][0])
            if rank is None:
                rank = 1
                ...
                rank = rank + 1
            self.Execute('''UPDATE rpg_version_1_0_1 SET rank = rank + 1 WHERE rank >= %i''' % (rank))    
            self.Execute('''INSERT INTO rpg_version_1_0_1 (id, steamid, name, xp, totalxp, level, credits, rank, language, automaticPopupClose) VALUES (DEFAULT, "%s", "%s", %i, %i, %i, %i, %i, "%s", %i)''' % (player.steamid, base64.encodestring(player.name), player.xp, player.totalxp, player.level, player.credits, rank, player.language, int(player.automaticPopupClose)))
    ...
    def Store(self, player):
        ...
        self.Execute('''SELECT rank, totalxp FROM rpg_version_1_0_1 WHERE steamid = "%s"''' % (steamid))
        data = self.Fetchall()

        oldrank = int(data[0][0])
        oldtotalxp = int(data[0][1])

        # If you gained XP
        if oldtotalxp < totalxp:
            # Add 1 from Everyone under your rank
            self.Execute('''UPDATE rpg_version_1_0_1 SET rank = rank + 1 WHERE rank < %i AND totalxp < %i''' % (oldrank, totalxp))
            # Get Rows Effected
            moved = self.cursor.rowcount
            moved = int(moved)
            es.msg(" > Moved: %i" % (moved))
            # Update the database with all new information including your new rank
            self.Execute('''UPDATE rpg_version_1_0_1 SET name="%s", xp=%i, totalxp=%i, level=%i, credits=%i, rank=rank-%i, language="%s", automaticPopupClose=%i, skills="%s" WHERE steamid="%s"''' % (base64.encodestring(player.name), player.xp, totalxp, player.level, player.credits, moved, player.language, int(player.automaticPopupClose), skills, steamid))
        # If you lost XP
        elif oldtotalxp > totalxp:
            # Subtract 1 from Everyone under your rank
            self.Execute('''UPDATE rpg_version_1_0_1 SET rank = rank - 1 WHERE rank > %i AND totalxp > %i''' % (oldrank, totalxp))
            # Get Your Rank
            moved = self.cursor.rowcount
            moved = int(moved)
            ...
            #Update the database with all new information including your new rank
            self.Execute('''UPDATE rpg_version_1_0_1 SET name="%s", xp=%i, totalxp=%i, level=%i, credits=%i, rank=rank+%i, language="%s", automaticPopupClose=%i, skills="%s" WHERE steamid="%s"''' % (base64.encodestring(player.name), player.xp, totalxp, player.level, player.credits, moved, player.language, int(player.automaticPopupClose), skills, steamid))
        # If your XP didn't change
        else:
            # WTF?
            self.Execute('''UPDATE rpg_version_1_0_1 SET name="%s", xp=%i, totalxp=%i, level=%i, credits=%i, language="%s", automaticPopupClose=%i, skills="%s" WHERE steamid="%s"''' % (base64.encodestring(player.name), player.xp, totalxp, player.level, player.credits, player.language, int(player.automaticPopupClose), skills, steamid))    
               
    def GetPlace(self, steamid):
        import MySQLdb
        # Get a place of the player with this steamid        
        self.Execute('''SELECT rank, totalxp, level, name FROM rpg_version_1_0_1 WHERE steamid = "%s"''' % (steamid))
        data = self.Fetchall()
        for i in data:
            return int(i[0]), int(i[1]), int(i[2]), str(i[3])
            
    def GetFix(self, steamid):
        import MySQLdb
        self.Execute('''SELECT rank, totalxp, level, name FROM rpg_version_1_0_1 WHERE steamid = "%s"''' % (steamid))
        data = self.Fetchall()
        for i in data:
            return int(i[0]), int(i[1]), int(i[2]), base64.decodestring(str(i[3]))

    def GetRank(self, place):
        import MySQLdb
        # Get ranking details of the player with this place 
        self.Execute('''SELECT name, totalxp, level FROM rpg_version_1_0_1 WHERE rank = %i''' % (place))
        data = self.Fetchall()
        for i in data:
            try:
    ...
    def Fetchall(self, args = ()):
        if args == ():
            ...
            return self.cursor.fetchall(args)
...
class Player(object): 
    ...
    class Properties(dict, object):   
        ...
        You can define default properties in the Properties.res file, which can be found at "cstrike/addons/eventscripts/infinityrpg/Includes". 
        ...
        def __getitem__(self, key):
            ...
            You can find default properties in the Properties.res file, which can be found at "cstrike/addons/eventscripts/infinityrpg/Includes".'''
    ...
    def Loaded(self):
        ...
        self.nextLevelXP =  250 + 50 * (self.level -1)
    ...
    def GetMaxLevel(self, skill, checkMax=True):
...
class Playerlist(object):
    ...
    You should not create any instance of this class. A instance called "playerlist" is available in the InfinityRPG core script.
...
class Eventhandler(object):
    ...
    def OnPlayerSpawn(self, ev):
        ...
        gamethread.delayed(0.1, player.player.setColor, (255, 255, 255, 255))
    ...
    def OnPlayerDeath(self, ev):
        ...
        if ev['attacker'] != ev['userid']:
            ...
            if True:
                xp = int(victim.GetLevel() * 25 * headshotMulti)         
            attacker.RaiseXP(xp)
            self.xpGaining[attacker.userid] += xp
    ...
    def OnRoundStart(self, ev):
        ...
        if config.GetBool('rpgDeathmatch'):
            gamethread.delayedname(30, 'rpgDeathmatchLoop', eventhandler.DeathmatchLoop, ())
...
class Commandhandler(object):
    # Events
    def Load(self):
        cmdlib.registerServerCommand('rpg_version', self.OnVersion, 'Return the version of InfinityRPG')
        cmdlib.registerServerCommand('rpg_load', self.OnLoad, 'Load a InfinityRPG skill')
        cmdlib.registerServerCommand('rpg_unload', self.OnUnload, 'Unload a InfinityRPG skill') 
        cmdlib.registerServerCommand('rpg_skills', self.OnSkills, 'Print all the InfinityRPG skills')
        cmdlib.registerServerCommand('rpg_debug', self.OnDebug, 'Debug the InfinityRPG savings') 
                ...
                for i in playerlist.GetPlayerlist():
                   if name in es.getplayername(i.userid).lower():
                        ...
                        if data != None: 
                            ...
                            xp = data[1]
                            level = data[2]
                            name = base64.decodestring(data[3])
    ...
    def OnDebug(self, args):
        # Debug the savings of InfinityRPG
...
# Messages      
def msg(text):
    '''Display a text to all players (like es.msg, but with a prefix for InfinityRPG).'''   
    es.msg('#multi', '#green[InfinityRPG]#default %s' %(text))
    
def tell(userid, text):
    '''Display a text to the player related to the userid (like es.tell, but with a prefix for InfinityRPG).'''   
    es.tell(userid, '#multi', '#green[InfinityRPG]#default %s' %(text))
    
def dbg(text):
    '''Display a text in the console of the server (like es.dbgmsg, but with a prefix for InfinityRPG).'''   
    es.dbgmsg(0, '[InfinityRPG] %s' %(text))
    
def echo(userid, text):
    '''Display a text in the console of the player related to the userid (like usermsg.echo, but with a prefix for InfinityRPG).'''   
    usermsg.echo(userid, '[InfinityRPG] %s' %(text))            
...
def load():     
    # Load rpg events
    es.loadevents('declare', 'addons/eventscripts/infinityrpg/Includes/rpg_events.res')    
    es.loadevents('addons/eventscripts/infinityrpg/Includes/rpg_events.res') 
    
    # Register client filter
    es.regclientcmd('rpg', 'infinityrpg/ClientFilter', 'All RPG Console commands')    
